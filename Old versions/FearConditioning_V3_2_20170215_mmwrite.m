...
function RunBehavior(handles)
    ...
    while (run == 1)
        ...
        if (handles.record_video)
            WriteFrames(is_sound_playing, is_shock_occurring, is_vns_occurring);
        end
...
function BeginVideo ( handles )
    ...
    file_name = [handles.rat_name '_' date_string '.avi'];
    ...
    %open(vid_writer);
    vid_writer = [path_name '\' file_name];
    mmwrite(vid_writer, 'Continue');
...
function WriteFrames (is_sound_playing, is_shock_occurring, is_vns_occurring)
    ...
    mmwrite(vid_writer, v, 'Continue', 'Initialized');
