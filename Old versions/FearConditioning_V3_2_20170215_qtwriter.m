...
function RunBehavior(handles)
    ...
    if (handles.record_video)
        vid_writer = BeginVideo(handles);
    end
    ...
    sound_intervals = randi([handles.isimin handles.isimax], 1, total_sounds);
    ...
    PlotSchedule(handles.session_axes, shock_schedule, handles.shockdur, sound_schedule, sound_duration, vns_schedule, 0);
    ...
    while (run == 1)
        ...
        if (handles.record_video)
            %WriteFrames(is_sound_playing, is_shock_occurring, is_vns_occurring, vid_writer);
        end
        ...
        if (~isempty(vns_schedule_queue))
            if (current_time >= vns_schedule_queue(1))
                ...
                is_vns_occurring = 1;
                vns_end_time = current_time + 1000;
                ToggleVNSTextBlockColor(handles, is_vns_occurring);
        ...
        PlotSchedule(handles.session_axes, shock_schedule, handles.shockdur, sound_schedule, sound_duration, vns_schedule, current_time);
...
function handles = Make_GUI(handles)
    ...
    handles.programlabel = uicontrol( ...
        ...
        'style', 'text', ...
        'string', 'Arduino Fear Conditioning V3.1', ...
        'fontweight', 'bold', ...
        'fontsize', 18, ...
...
function PlotSchedule ( session_axes, shock_schedule, shock_duration, sound_schedule, sound_duration, vns_schedule, current_time )
    ...
    max_xlim = max([(shock_schedule + shock_duration) (sound_schedule + sound_duration) (vns_schedule + 0.0167)]);
    if (isempty(max_xlim))
        max_xlim = 1;
    end
...
function vid_writer = BeginVideo ( handles )
    ...
    file_name = [handles.rat_name '_' date_string '.mov'];
    ...
    %open(vid_writer);
    vid_writer = QTWriter([path_name '\' file_name]);
...
function WriteFrames (is_sound_playing, is_shock_occurring, is_vns_occurring, vid_writer)
    ...
    %v = [];
    %v.frames = struct('cdata', {});
    %v.times = [];
    %v.width = 640;
    %v.height = 480;
    

    new_times = [last_end_time; times];
    diff_times = diff(new_times);
    last_end_time = times(end);
    ...
    for i = 1:size(frames, 4)
        ...
        new_frame = [];
        new_frame.cdata = uint8(frames(:, :, :, i));
        new_frame.colormap = [];
        
        dt = diff_times(i);
        fr = 1.0 / dt;
        if (~isinf(fr) && ~isnan(fr))
            vid_writer.FrameRate = fr;
        end
        
        writeMovie(vid_writer, new_frame);
        
        %v.frames(i) = new_frame;
        %v.times(i) = times(i);
...
function StopVideo ( vid_writer )

    global vid;

    stop(vid);
    
    %mmwrite(vid_writer, 'Initialized');
    close(vid_writer);
